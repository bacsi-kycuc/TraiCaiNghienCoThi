...
export default function PromptModal({
  ...
  const [giveawayMaxWinners, setGiveawayMaxWinners] = useState(1);
  const [drawDuration, setDrawDuration] = useState(10);
  const [linkVisibleDuration, setLinkVisibleDuration] = useState(10);
  const [giveawayStartDateTime, setGiveawayStartDateTime] = useState("");
  const [shouldResetGiveaway, setShouldResetGiveaway] = useState(false);
  ...
  useEffect(() => {
    if (editingPrompt) {
      ...
      setIsGiveaway(!!editingPrompt.isGiveaway);
      setGiveawayMaxWinners(editingPrompt.maxWinners || 1);
      setDrawDuration(editingPrompt.drawDuration || 10);
      setLinkVisibleDuration(editingPrompt.linkVisibleDuration || 10);
      
      // Convert ISO to datetime-local format (YYYY-MM-DDTHH:mm)
      if (editingPrompt.giveawayStartTime) {
        const d = new Date(editingPrompt.giveawayStartTime);
        const pad = (n: number) => n.toString().padStart(2, '0');
        const formatted = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
        setGiveawayStartDateTime(formatted);
      } else {
        setGiveawayStartDateTime("");
      }
      
      setShouldResetGiveaway(false);
    } else {
      setZone(currentZone);
      setTitle("");
      setGenre("");
      ...
      setIsGiveaway(false);
      setGiveawayMaxWinners(1);
      setDrawDuration(10);
      setLinkVisibleDuration(10);
      setShouldResetGiveaway(false);
    }
    ...
    onSave(
      {
        ...
        // Giveaway mechanism
        isGiveaway,
        maxWinners: isGiveaway ? giveawayMaxWinners : 1,
        drawDuration: isGiveaway ? drawDuration : 10,
        linkVisibleDuration: isGiveaway ? linkVisibleDuration : 10,
        giveawayStartTime: isGiveaway 
          ? (shouldResetGiveaway || !editingPrompt?.isGiveaway 
              ? (giveawayStartDateTime ? new Date(giveawayStartDateTime).toISOString() : new Date().toISOString()) 
              : (giveawayStartDateTime ? new Date(giveawayStartDateTime).toISOString() : (editingPrompt.giveawayStartTime || new Date().toISOString()))) 
          : undefined,
        participants: isGiveaway ? (shouldResetGiveaway || !editingPrompt?.isGiveaway ? [] : (editingPrompt.participants || [])) : undefined,
        winners: isGiveaway ? (shouldResetGiveaway || !editingPrompt?.isGiveaway ? [] : (editingPrompt.winners || [])) : undefined,
        giveawayStatus: isGiveaway ? (shouldResetGiveaway || !editingPrompt?.isGiveaway ? 'active' : (editingPrompt.giveawayStatus || 'active')) : undefined,
        drawnTime: isGiveaway ? (shouldResetGiveaway || !editingPrompt?.isGiveaway ? "" : (editingPrompt.drawnTime || "")) : undefined,
      },
      zone,
    );
    ...
    <div className="fixed inset-0 flex items-center justify-center z-[10000] p-4 animate-premium-backdrop">
      <div className="bg-[var(--card)] text-[var(--text)] rounded-3xl p-6 w-full max-w-[600px] shadow-2xl max-h-[90vh] flex flex-col justify-between overflow-hidden border-2 border-[var(--border)] animate-premium-modal">
        ...
        <div className="flex-1 overflow-y-auto pr-1.5 space-y-4">
          ...
          <div className="form-group border-t border-slate-100 dark:border-slate-700/50 pt-3 space-y-3">
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer font-bold text-xs text-slate-800 dark:text-slate-200 uppercase tracking-wide">
                <input
                  ...
                  className="accent-[var(--zone-primary)] w-4 h-4 cursor-pointer"
                />
                🎁 Kích hoạt hoạt động Give Away (Khu VIP)
              </label>
              
              {isGiveaway && editingPrompt && (
                <label className="flex items-center gap-1.5 cursor-pointer text-[10px] font-bold text-rose-500 uppercase">
                  <input
                    type="checkbox"
                    checked={shouldResetGiveaway}
                    onChange={() => setShouldResetGiveaway(!shouldResetGiveaway)}
                    className="accent-rose-500 w-3.5 h-3.5 cursor-pointer"
                  />
                  Chạy lại (Reset) Giveaway này
                </label>
              )}
            </div>
            ...
            {isGiveaway && (
              <div className="space-y-4">
                <div className="p-4 bg-purple-500/10 border border-purple-200/40 dark:border-purple-900/40 rounded-2xl animate-[in_0.2s_ease-out] space-y-4">
                  <div className="flex items-center gap-2 border-b border-purple-500/20 pb-2 mb-1">
                    <div className="w-1.5 h-4 bg-purple-500 rounded-full"></div>
                    <span className="text-[10px] font-bold text-purple-700 dark:text-purple-300 uppercase tracking-widest">
                      Cấu hình thời gian & Quy mô
                    </span>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2">
                      <label className="block font-bold mb-1.5 text-[10px] uppercase text-slate-500 dark:text-slate-400">
                        Thời gian bắt đầu (Scheduled)
                      </label>
                      <input
                        type="datetime-local"
                        value={giveawayStartDateTime}
                        onChange={(e) => setGiveawayStartDateTime(e.target.value)}
                        className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 rounded-xl outline-none text-xs focus:border-purple-500 transition-colors"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        min="1"
                        value={giveawayMaxWinners}
                        onChange={(e) => setGiveawayMaxWinners(Math.max(1, Number(e.target.value)))}
                        className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 rounded-xl outline-none text-xs focus:border-purple-500 transition-colors"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        min="1"
                        value={drawDuration}
                        onChange={(e) => setDrawDuration(Math.max(1, Number(e.target.value)))}
                        className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 rounded-xl outline-none text-xs focus:border-purple-500 transition-colors"
                      />
                    </div>
                    ...
                    <div className="col-span-2">
                      <label className="block font-bold mb-1.5 text-[10px] uppercase text-slate-500 dark:text-slate-400">
                        Thời gian hiện Link khám (phút)
                      </label>
                      <input
                        type="number"
                        min="1"
                        value={linkVisibleDuration}
                        onChange={(e) => setLinkVisibleDuration(Math.max(1, Number(e.target.value)))}
                        className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 rounded-xl outline-none text-xs focus:border-purple-500 transition-colors"
                      />
                    </div>
                  </div>
